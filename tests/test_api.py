...
def test_analyze_endpoint_phishing_url():
    response = client.post("/analyze", json={"url": "http://login.paypal.com.verify.tk/secure/update.php"})
    assert response.status_code == 200
    data = response.json()
...
def test_analyze_web_phishing_url():
    response = client.post("/analyze-web", data={"url": "http://kbzpay.workers.dev/login"})
    assert response.status_code == 200
